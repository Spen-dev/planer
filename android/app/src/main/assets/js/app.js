...
async function initDeviceSecret() {
  ...
  if (isDesktopShell() || isAndroidShell()) {
    const hasApi = isAndroidShell()
      ? Boolean(window.pywebview?.api?.get_device_secret)
...
async function ensureLicense() {
  if (!isDesktopShell() && !isAndroidShell()) return true;

  document.body.classList.add("boot-pending");

  const hasApi = isAndroidShell()
    ? Boolean(window.pywebview?.api?.check_license)
    : await waitForPyWebViewApi();
  if (!hasApi) {
...
function startApp() {
  ...
  if (isAndroidShell()) document.body.classList.add("planer-android");
  initCodeProtection();
  void bootApp();
}
